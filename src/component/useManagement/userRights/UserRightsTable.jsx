...
import { DeleteUserRightsAPI } from "../../../service/Api";
...
const UserRightsTable = ({
  ...
  const tableRights = rights || { rights_Update: true, rights_Delete: true };
  const { sortConfig, handleSort, applySorting } = useTableSort();

  const handleDelete = (row) => {
    showConfirm({
      title: "Delete User Rights",
      text: `Are you sure you want to delete user rights for "${row.userName}"?`,
      icon: "warning",
      confirmButtonText: "Yes, delete it!",
    }).then(async (result) => {
      if (result.isConfirmed) {
        try {
          const response = await DeleteUserRightsAPI(row.userRights_SK);
          if (response.status === 200) {
            showSuccess("Deleted!", "User rights have been deleted successfully.");
              ...
              <tbody className="divide-y divide-gray-100">
                {dataToRender.length === 0 ? (
                  <tr>
                    ...
                    >
                      No user rights found matching your search.
                    </td>
                  </tr>
                ) : (
                  dataToRender.map((row, index) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-3">
                        <span className="bg-orange-50 text-orange-600 px-2 py-1 rounded-lg font-medium text-[10px]">
                          {row.roleName}
                        </span>
                      </td>
                      ...
                      <td className="px-6 py-3">
                        <div className="flex items-center justify-center gap-2">
                          ...
                          {tableRights.rights_Update && (
                            <button
                              onClick={() => handleEdit(row)}
                              className="p-2 text-gray-600 bg-gray-50 hover:bg-gray-100 hover:text-blue-600 rounded-xl transition-all font-medium text-xs flex items-center gap-1.5"
                              title="Edit Rights"
                            >
                              <Edit size={14} />
